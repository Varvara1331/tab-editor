...
export const TabViewer: React.FC<TabViewerProps> = ({ tabData, title, artist, onComplete }) => {
  ...
  // Обновляем позицию воспроизведения и автоматическую прокрутку
  useEffect(() => {
    if (currentPosition && isMountedRef.current) {
      setPlayingPosition(currentPosition);
      
      // Автоматическая прокрутка к текущей ноте
    ...
    <div className="tab-viewer">
      <div className="tab-viewer-header">
        ...
        <div className="tab-viewer-controls">
          <button 
            className="tab-viewer-btn sound-btn"
            onClick={toggleSound}
          ...
          {!isPlaying ? (
            <button 
              className="tab-viewer-btn play-btn" 
              onClick={handlePlay} 
              disabled={!isSoundEnabled || isLoading}
            >
              <Play size={16} /> Проиграть
            </button>
          ) : (
            <button className="tab-viewer-btn pause-btn" onClick={handlePlay}>
              <Pause size={16} /> Пауза
            </button>
          )}
          <button className="tab-viewer-btn reset-btn" onClick={handleReset}>
            <RotateCcw size={16} /> Сброс
          </button>
        </div>
      </div>

      {/* Компактная табулатура - только одна струна */}
